...
De bron verschilt sterk per provincie. Slechts 1 van de 12 (Gelderland)
publiceert GS in dezelfde vergaderfeed als Provinciale Staten (Notubiz, met
een eigen vergadertype); de overige hebben geen vergaderportaal maar een
eigen website met besluitenlijst-PDF's — als eenvoudige index met directe
links (backend "pdf-index"), een jaar->maand-geneste index ("pdf-index-
genest"), een doorzoekbare Woo-index ("zeeland-website") of los
doorzoekbare per-besluit-pagina's ("zuid-holland-website"). Limburg heeft een
apart iBabs-portaal; Utrecht publiceert platte tekst zonder documentbijlagen
("utrecht-tekst"). Zie het geneste "gs"-veld per provincie in
...
Output: ~/Documents/notulen/gs/<naam>/
...
from api import (
    OUTPUT_BASIS, BRONNEN_MAP, GS_EIGEN_WEBSITE_BACKENDS,
    setup_logging, log, log_samenvatting, vraag_doorzoekbaar_maken, parse_jaren_arg,
    haal_vergaderingen_notubiz, download_vergaderingen_notubiz,
    haal_vergaderingen_ibabs_portaal, download_vergaderingen_ibabs,
...
def main():
    ...
    heeft_bron = any(gs_config.get(v) for v in ("notubiz_id", "ibabs_naam"))
    backend = gs_config.get("backend")
    if not heeft_bron and backend not in GS_EIGEN_WEBSITE_BACKENDS:
    ...
    notubiz_id = gs_config.get("notubiz_id")
    if notubiz_id:
    ...
    ibabs_naam = gs_config.get("ibabs_naam")
    if ibabs_naam:
